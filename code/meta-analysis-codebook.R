...
J <- 1-3/(4*(df$n - 2) - 1)
vd <- (df$n/(df$n/2)^2)+(df$d^2/(2*df$n))
vg <- J^2 * vd
...
vd <- df$n/(df$n1 * df$n2)+(df$d^2/(2*df$n))
...
setwd("C:/Users/s421506/tiu/research/effectsizes/")
write.table(dat, file = "codebook-meta-analyses-final.csv", row.names=F, col.names=T, sep=' ')
...
for (i in 1:nrow(dat)) {
  ...
  if (dat$disc.c[i] < 0.020) {
    dat$disccat.c[i] = 0
  } else if (dat$disc.c[i] >= 0.020 & dat$disc.c[i] <= 0.056) {
    dat$disccat.c[i] = 1
  } else if (dat$disc.c[i] >= 0.057 & dat$disc.c[i] <= 0.093) {
    dat$disccat.c[i] = 2
  } else if (dat$disc.c[i] >= 0.094) {
    dat$disccat.c[i] = 3
  } else {
  ...
  if (dat$disc.ci.li.c[i] < 0.020 | dat$disc.ci.ul.c[i] < 0.020) {
    dat$disccat.ci.c[i] = 0
  } else if (dat$disc.ci.li.c[i] >= 0.020 & dat$disc.ci.li.c[i] <= 0.056 | dat$disc.ci.ul.c[i] >= 0.020 & dat$disc.ci.ul.c[i] <= 0.056) {
    dat$disccat.ci.c[i] = 1
  } else if (dat$disc.ci.li.c[i] >= 0.057 & dat$disc.ci.li.c[i] <= 0.093 | dat$disc.ci.ul.c[i] >= 0.057 & dat$disc.ci.ul.c[i] <= 0.093) {
    dat$disccat.ci.c[i] = 2
  } else if (dat$disc.ci.li.c[i] >= 0.094 | dat$disc.ci.ul.c >= 0.094) {
  ...
  dat$outeff1.c[i] <- length(estdiff[estdiff >= 0.020 & estdiff <= 0.056])  # no of studies that have a small effect on MA effect size
  dat$outeff2.c[i] <- length(estdiff[estdiff >= 0.057 & estdiff <= 0.093])  # no of studies that have a medium effect on MA effect size
  dat$outeff3.c[i] <- length(estdiff[estdiff >= 0.094])                     # no of studies that have a large effect on MA effect size
  ...
  dat$outci1.c[i] <- max(length(ci.lb.diff[ci.lb.diff >= 0.020 & ci.lb.diff <= 0.056]),length(ci.ub.diff[ci.ub.diff >= 0.020 & ci.ub.diff <= 0.056])) # no of studies that have a small effect on MA effect size CI
  dat$outci2.c[i] <- max(length(ci.lb.diff[ci.lb.diff >= 0.057 & ci.lb.diff <= 0.093]),length(ci.ub.diff[ci.ub.diff >= 0.057 & ci.ub.diff <= 0.093])) # no of studies that have a medium effect on MA effect size CI
  dat$outci3.c[i] <- max(length(ci.lb.diff[ci.lb.diff >= 0.094]),length(ci.ub.diff[ci.ub.diff >= 0.094]))                                             # no of studies that have a large effect on MA effect size CI
  
  taudiff <- l1o$tau2 - c(res.cc$tau2) # tau2 estimate if ith study was omitted
  dat$outtau1.c[i] <- length(taudiff[taudiff >= 0.020 & taudiff <= 0.056])  # no of studies that have a small effect on tau2 estimate
  dat$outtau2.c[i] <- length(taudiff[taudiff >= 0.057 & taudiff <= 0.093])  # no of studies that have a medium effect on tau2 estimate
  dat$outtau3.c[i] <- length(taudiff[taudiff >= 0.094])                     # no of studies that have a large effect on tau2 estimate
  ...
  if (dat$disc.s[i] < 0.020) {
    dat$disccat.s[i] = 0
  } else if (dat$disc.s[i] >= 0.020 & dat$disc.s[i] <= 0.056) {
    dat$disccat.s[i] = 1
  } else if (dat$disc.s[i] >= 0.057 & dat$disc.s[i] <= 0.093) {
    dat$disccat.s[i] = 2
  } else if (dat$disc.s[i] >= 0.094) {
    dat$disccat.s[i] = 3
  } else {
    dat$disccat.s[i] = "check"
  }
  
  dat$disc.ci.li.s[i] <- res.co$ci.lb - res.cc$ci.lb  # difference subset original and complete checked CI LB of effect size estimate 
  dat$disc.ci.ul.s[i] <- res.co$ci.ub - res.cc$ci.ub  # difference subset original and complete checked CI UB of effect size estimate
  
  # Fill in discrepancy category for CI of effect sizes on subset checked MAs
  if (dat$disc.ci.li.s[i] < 0.020 | dat$disc.ci.ul.s[i] < 0.020) {
    dat$disccat.ci.s[i] = 0
  } else if (dat$disc.ci.li.s[i] >= 0.020 & dat$disc.ci.li.s[i] <= 0.056 | dat$disc.ci.ul.s[i] >= 0.020 & dat$disc.ci.ul.s[i] <= 0.056) {
    dat$disccat.ci.s[i] = 1
  } else if (dat$disc.ci.li.s[i] >= 0.057 & dat$disc.ci.li.s[i] <= 0.093 | dat$disc.ci.ul.s[i] >= 0.057 & dat$disc.ci.ul.s[i] <= 0.093) {
    dat$disccat.ci.s[i] = 2
  } else if (dat$disc.ci.li.s[i] >= 0.094 | dat$disc.ci.ul.s >= 0.094) {
  ...
  dat$outeff1.s[i] <- length(estdiff[estdiff >= 0.020 & estdiff <= 0.056])  # no of studies that have a small effect on MA effect size
  dat$outeff2.s[i] <- length(estdiff[estdiff >= 0.057 & estdiff <= 0.093])  # no of studies that have a medium effect on MA effect size
  dat$outeff3.s[i] <- length(estdiff[estdiff >= 0.094])                     # no of studies that have a large effect on MA effect size
  ...
  dat$outci1.s[i] <- max(length(ci.lb.diff[ci.lb.diff >= 0.020 & ci.lb.diff <= 0.056]),length(ci.ub.diff[ci.ub.diff >= 0.020 & ci.ub.diff <= 0.056])) # no of studies that have a small effect on MA effect size CI
  dat$outci2.s[i] <- max(length(ci.lb.diff[ci.lb.diff >= 0.057 & ci.lb.diff <= 0.093]),length(ci.ub.diff[ci.ub.diff >= 0.057 & ci.ub.diff <= 0.093])) # no of studies that have a medium effect on MA effect size CI
  dat$outci3.s[i] <- max(length(ci.lb.diff[ci.lb.diff >= 0.094]),length(ci.ub.diff[ci.ub.diff >= 0.094]))                                             # no of studies that have a large effect on MA effect size CI
  
  taudiff <- l1o$tau2 - c(res.sc$tau2) # tau2 estimate if ith study was omitted
  dat$outtau1.s[i] <- length(taudiff[taudiff >= 0.020 & taudiff <= 0.056])  # no of studies that have a small effect on tau2 estimate
  dat$outtau2.s[i] <- length(taudiff[taudiff >= 0.057 & taudiff <= 0.093])  # no of studies that have a medium effect on tau2 estimate
  dat$outtau3.s[i] <- length(taudiff[taudiff >= 0.094])                     # no of studies that have a large effect on tau2 estimate
